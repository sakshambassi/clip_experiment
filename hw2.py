...
def loss_func(logits, labels):
    ...
    #print(torch.stack(loss_list).mean())
...
def predict(logits):
    ...
    for x in range(logits.shape[0]):
      ...
      #print(logits)
      #a = input("check")
      a = logits[x] > logits[x][0]
      #print(torch.all(a == False))
      if torch.all(a == False):
        temp_list.append(0)
      for z in a.nonzero():
          temp_list.append(z.item())
          #print(z.item())
      
      list_return.append(temp_list)
      #raise NotImplementedError
    return list_return
...
for epoch in range(10):
    ...
    for step, inputs in enumerate(train_dataloader):
        ...
        labels = inputs['label']
        sample_list = torch.zeros(10, 11, dtype=int)
        for i,x in enumerate(labels):
            sample_list[i][x] = 1
